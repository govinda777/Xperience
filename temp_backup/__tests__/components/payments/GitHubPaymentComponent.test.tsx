...
const MockedGitHubPaymentProvider = GitHubPaymentProvider as jest.MockedClass<
  typeof GitHubPaymentProvider
>;
...
describe("GitHubPaymentComponent", () => {
  ...
  const mockPaymentResult = {
    ...
    paymentUrl:
      "https://github.com/sponsors/testuser/sponsorships?sponsor=testuser&frequency=one-time&amount=18&preview=true",
    amount: 18,
    ...
    MockedGitHubPaymentProvider.mockImplementation(
      () =>
        ({
          ...
          generatePaymentInstructions: jest
            .fn()
            .mockReturnValue(mockInstructions),
          convertUsdToBrl: jest.fn().mockReturnValue(99.0),
  ...
  describe("Renderização inicial", () => {
    it("deve renderizar estado de carregamento", () => {
      ...
      expect(
        screen.getByText("Preparando pagamento GitHub..."),
      ).toBeInTheDocument();
    });
    ...
    it("deve renderizar componente após inicialização", async () => {
      ...
      expect(
        screen.getByText("Patrocine via GitHub Sponsors"),
      ).toBeInTheDocument();
  ...
  describe("Exibição de informações", () => {
    ...
    it("deve exibir instruções de pagamento", () => {
      expect(
        screen.getByText("Como pagar via GitHub Sponsors"),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Clique no botão "Pagar com GitHub"/),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Você será redirecionado para o GitHub Sponsors/),
      ).toBeInTheDocument();
    });
    ...
    it("deve exibir notas importantes", () => {
      ...
      expect(
        screen.getByText(/🔒 Pagamento processado de forma segura pelo GitHub/),
      ).toBeInTheDocument();
    });
  ...
  describe("Interações do usuário", () => {
    ...
    it("deve permitir confirmação manual", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("✅ Já paguei - Confirmar manualmente"),
        ).toBeInTheDocument();
      });
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Obrigado pelo seu patrocínio/),
        ).toBeInTheDocument();
      });

      expect(defaultProps.onPaymentComplete).toHaveBeenCalledWith(
        mockPaymentResult,
      );
  ...
  describe("Acessibilidade", () => {
    ...
    it("deve ter informações de segurança visíveis", () => {
      ...
      expect(
        screen.getByText(/Seus dados estão protegidos/),
      ).toBeInTheDocument();
    });
  });
});
