...
describe('PixPaymentComponent', () => {
  ...
  it('should display pix benefits', () => {
    ...
    expect(screen.getByText('Confirmação instantânea • Disponível 24/7')).toBeInTheDocument();
    ...
    expect(screen.getByText('Após confirmar o pagamento PIX, seu acesso será liberado imediatamente.')).toBeInTheDocument();
  });
  ...
  it('should toggle instructions when clicked', () => {
    ...
    const instructionsButton = screen.getByText('Como funciona o pagamento PIX?');
    
    // Instructions should not be visible initially
    expect(screen.queryByText('Clique em "Gerar QR Code PIX" abaixo')).not.toBeInTheDocument();

    // Click to show instructions
    fireEvent.click(instructionsButton);
    expect(screen.getByText('Clique em "Gerar QR Code PIX" abaixo')).toBeInTheDocument();
    expect(screen.getByText('Abra o app do seu banco e escaneie o QR Code')).toBeInTheDocument();
    expect(screen.getByText('Confirme o pagamento no seu app bancário')).toBeInTheDocument();
    expect(screen.getByText('Seu acesso será liberado automaticamente')).toBeInTheDocument();
  ...
  it('should disable button when disabled prop is true', () => {
    ...
    const paymentButton = screen.getByText('Gerar QR Code PIX').closest('button');
  ...
  it('should disable button when processing', () => {
    ...
    const paymentButton = screen.getByText('Gerando QR Code PIX...').closest('button');
    expect(paymentButton).toBeDisabled();
  });
  ...
  it('should display expiration information', () => {
    ...
    expect(screen.getByText('O QR Code PIX expira em 15 minutos após a geração')).toBeInTheDocument();
  });
  ...
  it('should display compatible banks', () => {
    ...
    expect(screen.getByText('Bancos e carteiras compatíveis:')).toBeInTheDocument();
