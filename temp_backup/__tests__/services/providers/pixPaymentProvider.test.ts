...
describe('PixPaymentProvider', () => {
  ...
  describe('validatePaymentData', () => {
    ...
    it('should validate minimum amount', () => {
      ...
      const result = provider.validatePaymentData(invalidData);
      expect(result.isValid).toBe(false);
      expect(result.errors.some(error => error.includes('Minimum amount'))).toBe(true);
    });
    ...
    it('should validate maximum amount', () => {
      ...
      const result = provider.validatePaymentData(invalidData);
      expect(result.isValid).toBe(false);
      expect(result.errors.some(error => error.includes('Maximum amount'))).toBe(true);
    });
    ...
    it('should validate currency', () => {
      ...
      const result = provider.validatePaymentData(invalidData);
      expect(result.isValid).toBe(false);
      expect(result.errors.some(error => error.includes('Currency USD not supported'))).toBe(true);
    });
  });

  describe('formatAmount', () => {
    it('should format amount correctly', () => {
      expect(provider.formatAmount(100.00, 'BRL')).toBe('R$ 100,00');
