...
import { PaymentError, PaymentProviderInterface, PaymentResult, PaymentStatus } from '../../types/payment';
...
describe('PaymentService', () => {
  ...
  describe('Payment Processing', () => {
    ...
    test('should throw error for non-existent provider', async () => {
      await expect(
        paymentService.processPayment('bitcoin', 100, 'BRL', 'plan-1', 'user-1')
      ).rejects.toThrow(PaymentError);
    });

    test('should handle provider processing error', async () => {
      (mockProvider.process as jest.Mock).mockRejectedValue(new Error('Provider error'));
      (fetch as jest.Mock).mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({ bitcoin: { brl: 300000 } })
      });
    ...
    test('should convert currency when needed', async () => {
      ...
      await paymentService.processPayment('pix', 100, 'BTC', 'plan-1', 'user-1');

      // Verifica se a conversão foi feita
      expect(mockProvider.process).toHaveBeenCalledWith(
        expect.any(Number), 'plan-1', 'user-1'
      );
    });
  });
  ...
  describe('Payment Verification', () => {
    ...
    test('should handle verification error', async () => {
      (mockProvider.verify as jest.Mock).mockRejectedValue(new Error('Verification failed'));
  ...
  describe('Payment Cancellation', () => {
    ...
    test('should handle cancellation error gracefully', async () => {
      (mockProvider.cancel as jest.Mock).mockRejectedValue(new Error('Cancel failed'));
