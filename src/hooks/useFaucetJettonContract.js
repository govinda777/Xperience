...
export function useFaucetJettonContract() {
    ...
    const faucetJettonContract = useAsyncInitialize(async () => {
        if (!client || !wallet)
            return undefined;
        try {
            const contract = new FaucetJetton(Address.parse("EQB8StgTQXidy32a8xfu7j4HMoWYV0b0cFM8nXsP2cza_b7Y"));
            return {
    ...
    const jwContract = useAsyncInitialize(async () => {
        if (!faucetJettonContract || !client || !wallet)
            return undefined;
        try {
            const walletAddress = Address.parse(wallet);
            const jettonWalletAddress = await faucetJettonContract.getWalletAddress(walletAddress);
            ...
            return {
                ...
                async sendTransfer(via, to, amount) {
                    const message = beginCell()
