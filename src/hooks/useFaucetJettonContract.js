...
export function useFaucetJettonContract() {
    ...
    const faucetJettonContract = useAsyncInitialize(async () => {
        if (!client || !wallet)
            return;
        const contract = new FaucetJetton(Address.parseFriendly("EQB8StgTQXidy32a8xfu7j4HMoWYV0b0cFM8nXsP2cza_b7Y").address);
        return {
            address: contract.address,
            async getWalletAddress(owner) {
                const state = await client.getContractState(contract.address);
                return state.state === "active" ? state.balance.toString() || "" : "";
            },
            async sendMintFromFaucet(via, to) {
    ...
    const jwContract = useAsyncInitialize(async () => {
        if (!faucetJettonContract || !client)
            return;
        const jettonWalletAddress = await faucetJettonContract.getWalletAddress(Address.parseFriendly(wallet).address);
