...
describe('useFaucetJettonContract', () => {
    ...
    it('should initialize faucet jetton contract', async () => {
        ...
        await act(async () => {
            const fn = useAsyncInitialize.mock.calls[0][0];
        ...
        expect(useAsyncInitialize).toHaveBeenCalledWith(expect.any(Function), [mockClient, mockWalletAddress]);
    });
    ...
    it('should initialize jetton wallet contract', async () => {
        ...
        const { result } = renderHook(() => useFaucetJettonContract());
        await act(async () => {
            const fn = useAsyncInitialize.mock.calls[1][0];
            const contract = await fn();
    ...
    it('should mint tokens', async () => {
        ...
        useAsyncInitialize.mockReturnValueOnce(mockFaucetContract);
        const { result } = renderHook(() => useFaucetJettonContract());
        await act(async () => {
            result.current.mint();
        });
        expect(mockFaucetContract.sendMintFromFaucet).toHaveBeenCalledWith(mockSender, expect.objectContaining({
    ...
    it('should not mint tokens when contract is not initialized', async () => {
        useAsyncInitialize.mockReturnValue(null);
        const { result } = renderHook(() => useFaucetJettonContract());
        await act(async () => {
            result.current.mint();
        });
