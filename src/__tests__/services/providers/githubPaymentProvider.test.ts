...
describe('GitHubPaymentProvider', () => {
  ...
  describe('Configuração', () => {
    ...
    it('deve detectar configuração inválida', () => {
      ...
      expect(validation.errors).toContain('Nome de usuário do GitHub não configurado');
    });
  ...
  describe('Webhook', () => {
    ...
    it('deve rejeitar webhook com ação não suportada', async () => {
      ...
      await expect(provider.processWebhook(payload)).rejects.toThrow('Ação de webhook não suportada');
    });
  ...
  describe('Geração de URL', () => {
    it('deve gerar URL do GitHub Sponsors corretamente', async () => {
      ...
      expect(result.paymentUrl).toContain('github.com/sponsors/testuser/sponsorships');
  ...
  describe('Tratamento de erros', () => {
    it('deve lidar com erros no processamento', async () => {
      ...
      await expect(invalidProvider.process(100, 'plan-1', 'user-1')).rejects.toThrow();
    });
  });
});
