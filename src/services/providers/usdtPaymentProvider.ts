...
export class USDTPaymentProvider implements PaymentProviderInterface {
  ...
  constructor() {
    this.rpcUrl = process.env.ETHEREUM_RPC_URL || 'https://mainnet.infura.io/v3/YOUR_PROJECT_ID';
   ...
   */
  private async generatePaymentAddress(userId: string, planId: string): Promise<EthereumAddress> {
    try {
   ...
   */
  private async getUSDTTransactions(address: string): Promise<USDTTransaction[]> {
    try {
   ...
   */
  private async makeRPCCall(method: string, params: any[]): Promise<EthereumRPCResponse> {
