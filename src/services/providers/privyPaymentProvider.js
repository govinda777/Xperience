...
export class PrivyPaymentProvider {
    id = "pix"; // Using PIX as the primary method
    name = "Privy Payment Gateway";
    type = "fiat";
    supportedCurrencies = ["BRL", "USD", "BTC", "USDT"];
    ...
    async processPixPayment(amount, planId, userId, customerInfo) {
        try {
    ...
    async processBitcoinPayment(amount, planId, userId) {
        try {
    ...
    async processUSDTPayment(amount, planId, userId, network = "ethereum") {
        try {
    ...
    async processGitHubPayment(amount, planId, userId, githubUsername) {
        try {
