...
export class SEOService {
    ...
    try {
      ...
      if (seoConfig.isDev) {
        const mockMetrics: SEOMetrics = {
          ...
          searchQueries: [
            { query: 'mentoria empresarial', impressions: 1200, clicks: 85, ctr: 7.1, position: 3.2 },
            { query: 'consultoria para empreendedores', impressions: 980, clicks: 62, ctr: 6.3, position: 4.1 },
            { query: 'IA do empreendedor', impressions: 750, clicks: 95, ctr: 12.7, position: 2.1 }
          ]
        };
  ...
  async auditPagePerformance(url: string): Promise<PerformanceAudit | null> {
    ...
    try {
      ...
      // const apiKey = seoConfig.pageSpeedApiKey;
