...
export class UserWalletService {
    walletService;
    localStorageKey = process.env.NODE_ENV === 'test' ? testConfig.testKeys.wallet : "user_wallet_data";
    recoveryKeyPrefix = "recovery_";
    constructor() {
     ...
     */
    async generateRecoveryKey(userId, password) {
        try {
            ...
            localStorage.setItem(`${this.recoveryKeyPrefix}${userId}`, encryptedWithRecovery);
            return recoveryKey;
        }
        catch (error) {
     ...
     */
    async recoverWallet(userId, recoveryKey, newPassword) {
        try {
