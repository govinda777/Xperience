...
const PlansTable: React.FC<PlansTableProps> = ({ type, plans }) => {
  ...
  return (
    <div
      className="w-full overflow-x-auto py-10 flex justify-center"
      style={{ fontFamily: "Quicksand, Poppins, Inter, sans-serif" }}
    >
      <div className="rounded-2xl shadow-2xl bg-white w-full max-w-[1200px] border border-[#F2F2F2]">
        ...
        >
          <thead>
            <tr>
              <th
                className={`sticky left-0 z-20 py-6 px-4 text-xl font-extrabold text-black bg-[#FFF1E7] rounded-tl-2xl align-middle shadow-md ${cellBorder}`}
              >
                CATEGORIA
              </th>
              {plans.map((plan, idx) => (
                ...
                >
                  <div className="flex flex-col items-center justify-center">
                    ...
                    {plan.isRecomendad && (
                      <span
                        className="mt-2 px-3 py-1 bg-[#F34A0D] text-white text-xs font-bold rounded-full shadow"
              ...
              {hasNinja && (
                <th
                  className={`py-6 px-4 text-xl font-extrabold align-middle bg-white text-center rounded-tr-2xl ${cellBorder}`}
                  style={{ color: "#F34A0D", fontWeight: 800 }}
                >
                  NINJA
                </th>
          ...
          <tbody>
            ...
            <tr style={{ background: "#FFFDF8" }}>
              <td
                className={`sticky left-0 z-10 py-6 px-4 font-bold text-black bg-[#FFF1E7] align-middle shadow-md ${cellBorder}`}
              >
                INVESTIMENTO
              </td>
              {plans.map((plan) => (
                <td
                  key={plan.title}
                  className={`py-6 px-4 text-[#1A1A1A] align-middle text-center bg-white ${cellBorder}`}
                >
            ...
            {moduleNames.map((mod, i) => (
              <tr
                key={mod}
                style={{ background: i % 2 === 0 ? "#FFFDF8" : "#F9F6F1" }}
              >
                <td
                  className={`sticky left-0 z-10 py-6 px-4 font-bold text-black bg-[#FFF1E7] align-middle shadow-md ${cellBorder}`}
                >
