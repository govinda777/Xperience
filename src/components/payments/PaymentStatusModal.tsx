...
import { PaymentResult, PaymentStatus, PaymentProvider } from '../../types/payment';
            ...
            {(payment.paymentAddress || payment.qrCode) && (
              <div className="mb-4">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <button
                    onClick={() => copyToClipboard(payment.paymentAddress || payment.qrCode || '')}
                    className={`
            ...
            {method === 'bitcoin' && (
              <div className="mb-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                <p className="text-sm text-orange-800">
                  <strong>Importante:</strong> Envie exatamente ₿ {payment.amount.toFixed(8)}. 
                  A confirmação pode levar de 10 a 30 minutos.
                </p>
              </div>
            )}
            ...
            {method === 'usdt' && (
              <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                <p className="text-sm text-green-800">
                  <strong>Rede:</strong> {payment.metadata?.networkId === 1 ? 'Ethereum' : 'Polygon'}<br/>
                  <strong>Valor:</strong> Exatamente ${payment.amount.toFixed(2)} USDT
                </p>
              </div>
            )}
          </div>
        ...
        {status === 'completed' && (
          <div className="p-6 text-center">
            ...
            <p className="text-gray-600 mb-4">
              Seu acesso foi liberado com sucesso. Você já pode começar a usar todos os recursos do seu plano.
            </p>
            <button
              onClick={onClose}
        ...
        {(status === 'pending' || status === 'processing') && (
          <div className="p-6 border-t border-gray-200 space-y-2">
            ...
            <p className="text-xs text-gray-500 text-center">
              Você pode fechar esta janela e o pagamento continuará sendo monitorado
            </p>
          </div>
        )}
      </div>
