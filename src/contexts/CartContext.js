...
const cartReducer = (state, action) => {
    switch (action.type) {
        ...
        case "ADD_ITEM": {
            const existingItemIndex = state.cart.items.findIndex((item) => item.planId === action.payload.planId);
            let newItems;
...
export const CartProvider = ({ children }) => {
    ...
    const createCheckoutSession = async (customerInfo, billingAddress) => {
        try {
            dispatch({ type: "SET_LOADING", payload: true });
            dispatch({ type: "SET_ERROR", payload: null });
