...
import {
  ...
  MockBitcoinProvider,
  MockUSDTProvider,
} from "../../src/__tests__/mocks/paymentMocks";
...
Given(
  "que existe um pagamento PIX com ID {string}",
  async function (transactionId: string) {
    ...
    const paymentData = {
      [transactionId]: {
        ...
        status: "pending",
        metadata: {
          externalReference: testConfig.testTransactions.pix.externalReference,
          preferenceId: testConfig.testTransactions.pix.preferenceId,
        },
...
Given(
  "que existe um pagamento Bitcoin com ID {string}",
  async function (transactionId: string) {
    world.transactionId = transactionId;
    ...
    const paymentData = {
      [transactionId]: {
        ...
        status: "pending",
        metadata: {
          paymentAddress: "test-btc-address",
          btcAmount: 0.001,
...
Given(
  "que existe um pagamento USDT com ID {string}",
  async function (transactionId: string) {
    world.transactionId = transactionId;
    ...
    const paymentData = {
      [transactionId]: {
        ...
        status: "pending",
        metadata: {
          paymentAddress: testConfig.testTransactions.crypto.paymentAddress,
          usdtAmount: 100,
          createdAt: Date.now() - 60000,
        },
        createdAt: new Date().toISOString(),
...
Given(
  "que o pagamento está com status {string}",
...
Given(
  "que a transação foi enviada mas tem {int} confirmações",
  async function (confirmations: number) {
        ...
        Promise.resolve([
          {
            ...
            vout: [
              {
                scriptpubkey_address: "test-btc-address",
...
Given(
  "que a transação tem pelo menos {int} confirmação",
  async function (minConfirmations: number) {
...
Given(
  "que a transação tem {int} confirmações na rede Ethereum",
  async function (confirmations: number) {
    world.confirmations = confirmations;
        ...
        json: () =>
          Promise.resolve({
            ...
            result: [
              {
                hash: "0xabc123def456",
                from: "test-eth-from-address",
                to: "test-eth-to-address",
...
Given(
  "que a transação tem {int} ou mais confirmações",
  async function (minConfirmations: number) {
...
Given("que o tempo de expiração foi ultrapassado", async function () {
  ...
  if (paymentData[world.transactionId]) {
    paymentData[world.transactionId].expiresAt = new Date(
      Date.now() - 60000,
    ).toISOString();
    mockStorage["xperience_payments"] = JSON.stringify(paymentData);
  }
...
Given(
  "que não existe um pagamento com ID {string}",
  async function (transactionId: string) {
    world.transactionId = transactionId;
...
When("eu verificar o status do pagamento", async function () {
  try {
    const provider = world.transactionId.startsWith("pix")
      ? "pix"
      : world.transactionId.startsWith("btc")
        ? "bitcoin"
        : "usdt";
...
Then(
  "o status deve permanecer {string}",
...
Then(
  "o status deve ser atualizado para {string}",
  async function (expectedStatus: string) {
    expect(world.verificationResult).toBe(expectedStatus);
  },
);
...
Then(
  "deve ser exibido o número de confirmações \\({int}/{int}\\)",
  async function (current: number, required: number) {
...
Then(
  "deve ser mostrada uma mensagem explicativa sobre confirmações",
  async function () {
    expect(world.verificationResult).toBe("processing");
  },
);
...
Then(
  "deve ser mostrada uma estimativa de tempo para confirmação final",
  async function () {
    expect(world.verificationResult).toBe("processing");
  },
);
...
Then(
  "deve ser oferecida a opção de criar um novo pagamento",
  async function () {
    expect(world.verificationResult).toBe("expired");
  },
);
...
Then(
  "deve ser oferecida a opção de tentar verificar novamente",
  async function () {
    expect(world.verificationResult).toBe("failed");
  },
);
...
Then(
  "deve ser exibida uma mensagem de pagamento não encontrado",
  async function () {
