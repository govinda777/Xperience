...
const testConfig = JSON.parse(
  fs.readFileSync(
    path.resolve(__dirname, '../../test-data/auth-test-config.json'),
    'utf8'
  )
);
...
// Helper functions
const simulateAuth0Login = (credentials: {
  email: string;
  password: string;
}) => {
  const validUser = testConfig.testUsers.valid;
  if (
...
When("insiro minhas credenciais válidas", function () {
  // Use valid credentials from config
  this.credentials = testConfig.testUsers.valid;
});

When("insiro credenciais inválidas", function () {
  // Use invalid credentials from config
  this.credentials = testConfig.testUsers.invalid;
});
