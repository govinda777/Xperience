...
Given(
  "I have the {string} plan in my cart with quantity {int}",
  function (planName: string, quantity: number) {
    const plan = mockPlans[planName as keyof typeof mockPlans];
    if (!plan) {
      throw new Error(`Plan ${planName} not found`);
    }
...
Given(
  "I have a plan with {int}% discount in my cart",
  function (discountPercentage: number) {
    if (!currentCart) {
      currentCart = cartService.createCart();
    }
...
When(
  "I try to add more than {int} of the same plan",
...
Then(
  "my cart should contain {int} different item(s)",
...
Then(
  "the cart should show {int} unique item",
  function (expectedCount: number) {
    expect(currentCart.items.length).toBe(expectedCount);
  },
);
...
Then(
  "the {string} plan should not be in the cart",
  function (planName: string) {
    const plan = mockPlans[planName as keyof typeof mockPlans];
    const itemExists = currentCart.items.some(
      (item: CartItem) => item.id === plan.id,
    );
...
Then(
  "the quantity should remain at {int}",
  function (expectedQuantity: number) {
    const plan = mockPlans.Essencial;
