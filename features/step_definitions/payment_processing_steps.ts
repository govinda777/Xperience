...
Given(
  "que eu selecionei o plano {string} por R$ {float}",
  async function (planName: string, price: number) {
    world.selectedPlan = {
...
Given(
  "que eu escolhi o método de pagamento {string}",
...
Given(
  "que a cotação do Bitcoin está em R$ {float}",
...
Given(
  "que a cotação do USDT está em R$ {float}",
  async function (price: number) {
...
Given(
  "que o provedor PIX demora mais de {int} segundos para responder",
...
Then(
  "o status do pagamento deve ser {string}",
...
Then(
  "deve haver um prazo de expiração de {int} minutos",
...
Then("deve ser gerado um endereço Bitcoin único", async function () {
  ...
  expect(world.paymentResult.paymentAddress).toMatch(
    /^[13][a-km-zA-HJ-NP-Z1-9]{25,34}$/,
  );
});

Then(
  "deve ser calculado o valor em BTC \\({float} BTC\\)",
  async function (expectedBtc: number) {
    expect(world.paymentResult).toBeTruthy();
...
Then("deve ser gerado um QR Code Bitcoin", async function () {
  ...
  expect(world.paymentResult.qrCode).toContain(
    world.paymentResult.paymentAddress,
  );
});

Then(
  "deve haver um prazo de expiração de {int} hora",
...
Then(
  "deve ser calculado o valor em USDT \\({int} USDT\\)",
  async function (expectedUsdt: number) {
    expect(world.paymentResult).toBeTruthy();
...
Then(
  "o erro deve conter o código {string}",
  async function (expectedCode: string) {
    expect(world.error).toBeTruthy();
