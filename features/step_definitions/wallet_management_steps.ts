...
const mockWalletService = {
  createWallet: (userId: string): WalletData => ({
    ...
    smartAccountAddress: '0xabcdef123456789',
    balance: '0',
  }),
...
Given('que sou um novo usuário autenticado', function () {
  authenticationState.isAuthenticated = true;
  authenticationState.user = { sub: 'new-user-456', email: 'newuser@email.com' };
  walletData = null;
  transactions = [];
});
...
Given('que realizei várias transações', function () {
  transactions = [
    {
      hash: '0xabc123',
      to: '0x742d35Cc6634C0532925a3b8D4C9db4C0532925a',
      value: '0.5',
      status: 'completed',
    ...
    {
      hash: '0xdef456',
      to: '0x852d35Cc6634C0532925a3b8D4C9db4C0532925b',
      value: '0.3',
      status: 'completed',
...
When('clico em {string}', function (buttonText: string) {
  if (buttonText === 'Enviar' && currentTransaction) {
    try {
      lastTransactionHash = mockWalletService.sendTransaction(currentTransaction);
      errorMessage = null;
    } catch (error) {
...
When('eu preencho o formulário de transação com endereço inválido:', function (dataTable: any) {
  const data = dataTable.hashes()[0];
  currentTransaction = {
    to: data.Destinatário,
    value: data.Valor,
...
Then('a transação deve aparecer no histórico', function () {
  const recentTransaction = transactions.find(tx => tx.hash === lastTransactionHash);
  expect(recentTransaction).toBeDefined();
});

Then('devo ver uma mensagem de erro {string}', function (expectedError: string) {
...
Then('o indicador de carregamento deve ser exibido durante a atualização', function () {
...
Then('devo ver indicadores de carregamento apropriados:', function (dataTable: any) {
...
Then('a interface deve se adaptar ao tamanho da tela:', function (dataTable: any) {
