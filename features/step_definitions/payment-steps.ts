...
Given(
  "os métodos de pagamento estão disponíveis",
...
Given(
  "que selecionei o plano {string} de R$ {float}",
  function (this: CustomWorld, planName: string, price: number) {
...
When(
  "eu escolho o método de pagamento {string}",
  function (this: CustomWorld, method: string) {
...
Then("devo ver as informações de pagamento", function (this: CustomWorld) {
  ...
  expect(
    screen.getByText(`R$ ${this.paymentData.amount.toFixed(2)}`),
  ).toBeInTheDocument();
});

Then(
  "devo ver o tempo de expiração de {int} minutos",
  function (this: CustomWorld, minutes: number) {
    expect(
      screen.getByText(
        `O QR Code PIX expira em ${minutes} minutos após a geração`,
      ),
    ).toBeInTheDocument();
  },
);
...
Then(
  "devo ver o desconto de {int}% aplicado",
  function (this: CustomWorld, discount: number) {
    expect(screen.getByText(`${discount}% OFF`)).toBeInTheDocument();
  },
);

Then(
  "o valor final deve ser R$ {float}",
  function (this: CustomWorld, finalPrice: number) {
...
Then(
  "devo ver a rede selecionada {string}",
  function (this: CustomWorld, network: string) {
...
Then(
  "meu acesso deve ser liberado imediatamente",
  function (this: CustomWorld) {
...
// Validation steps
Given(
  "que estou preenchendo os dados de pagamento",
  function (this: CustomWorld) {
...
Then(
  "o botão de pagamento deve estar desabilitado",
  function (this: CustomWorld) {
...
Then(
  "o botão de pagamento deve estar habilitado",
  function (this: CustomWorld) {
    const button = screen.queryByRole("button", { name: /gerar|pagar/i });
    if (button) {
...
Given(
  "que escolhi o método de pagamento {string}",
  function (this: CustomWorld, method: string) {
    const methodMap: Record<string, string> = {
      PIX: "pix",
      Bitcoin: "bitcoin",
...
Then(
  "o processo de pagamento deve ser reiniciado",
  function (this: CustomWorld) {
...
When(
  "o tempo de expiração de {int} minutos é atingido",
  function (this: CustomWorld, minutes: number) {
    this.setPaymentData({
      ...this.paymentData,
      expired: true,
...
Then(
  "devo ser redirecionado para a página de sucesso",
  function (this: CustomWorld) {
    expect(this.paymentData.paymentStatus).toBe("confirmed");
  },
);
