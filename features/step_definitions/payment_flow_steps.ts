...
// Mock data
const plans = {
  'Essencial': { id: 'essential', name: 'Essencial', price: 297.00 },
  'Expert': { id: 'expert', name: 'Expert', price: 497.00 },
  'Premium': { id: 'premium', name: 'Premium', price: 797.00 },
};
...
const calculatePrice = (basePrice: number, method: string): number => {
  const paymentMethod = Object.values(paymentMethods).find(pm => pm.id === method);
...
const simulatePaymentProcessing = (method: string, amount: number): PaymentData => {
  ...
  switch (method) {
    ...
    case 'usdt':
      return {
        ...
        status: 'pending',
        transactionId,
        address: '0x742d35Cc6634C0532925a3b8D4C9db4C0532925a',
        qrCode: 'usdt-qr-code-data',
        expiresAt,
      };
    case 'github':
      return {
...
Given('escolhi {string} como método de pagamento', function (methodName: string) {
...
Given('que iniciei um processo de pagamento', function () {
  ...
  paymentData = simulatePaymentProcessing('pix', 297.00);
...
When('clico em {string}', function (buttonText: string) {
  if (buttonText === 'Assinar Agora') {
    ...
    if (selectedPlan && selectedPaymentMethod) {
      paymentData = simulatePaymentProcessing(selectedPaymentMethod, selectedPlan.price);
    }
...
When('seleciono {string} como método de pagamento', function (methodName: string) {
  const method = paymentMethods[methodName as keyof typeof paymentMethods];
  expect(method).toBeDefined();
  selectedPaymentMethod = method.id;
});
...
When('o tempo de expiração é atingido', function () {
  if (paymentData && paymentData.expiresAt && paymentData.expiresAt < new Date()) {
...
When('após a confirmação na blockchain o status deve mudar para {string}', function (status: string) {
  if (paymentData && paymentData.status === 'processing') {
    paymentData.status = status === 'Pagamento Confirmado' ? 'completed' : status;
  }
});

When('após a confirmação o status deve mudar para {string}', function (status: string) {
  if (paymentData && paymentData.status === 'processing') {
    paymentData.status = status === 'Pagamento Confirmado' ? 'completed' : status;
  }
});
...
Then('devo ver o preço {string}', function (expectedPrice: string) {
  if (selectedPlan && selectedPaymentMethod) {
    const calculatedPrice = calculatePrice(selectedPlan.price, selectedPaymentMethod);
...
Then('devo ver o preço com desconto de {int}%', function (discountPercent: number) {
  if (selectedPlan && selectedPaymentMethod) {
    const originalPrice = selectedPlan.price;
    const discountedPrice = calculatePrice(originalPrice, selectedPaymentMethod);
